...
function debuff:Cast(elixir)
    ...
    if self.debuffCooldown and self.debuffCooldown > mq.gettime() then return "on debuff cooldown" end
    ...
    if elixir.Config.IsDebuffSubtleCasting and mq.TLO.Me.PctAggro() > 80 then
    ...
    for i = 1, mq.TLO.Me.NumGems() do
        if elixir.Gems[i].Tag.IsDebuff and
            ...
            elixir:DebugPrintf("found debuff at gem %d will cast on %d", i, spawn.ID())
            isCasted, lastCastOutput = debuff:CastGem(elixir, spawn.ID(), i)
            elixir.Gems[i].Output = elixir.Gems[i].Output .. " debuff ai: " .. lastCastOutput

...
function nuke:Cast(elixir)
    ...
    if self.nukeCooldown and self.nukeCooldown > mq.gettime() then return "on nuke cooldown" end
    ...
    if elixir.Config.IsNukeSubtleCasting and mq.TLO.Me.PctAggro() > 80 then
    ...
    for i = 1, mq.TLO.Me.NumGems() do
        if elixir.Gems[i].Tag.IsNuke and
            ...
            elixir:DebugPrintf("found nuke at gem %d will cast on %d", i, spawn.ID())
            isCasted, lastCastOutput = nuke:CastGem(elixir, spawn.ID(), i)
            elixir.Gems[i].Output = elixir.Gems[i].Output .. " nuke ai: " .. lastCastOutput
